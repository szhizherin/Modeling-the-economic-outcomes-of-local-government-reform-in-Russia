...
y_names <- c(# "Общий объем расходов бюджета муниципального образования|||t8313001|||Всего",
             ...
             "Кредиторская задолженность (по okved2)|||t8942005|||Муниципальная собственность_Всего по обследуемым видам экономической деятельности",
             ...
             "Количество организаций, представивших отчет (по 2016 год)|||t8042010|||Муниципальная собственность_Всего",
             "Количество организаций, представивших отчет (по okved2)|||t8942010|||Муниципальная собственность_Всего по обследуемым видам экономической деятельности",
             "Прибыль (убыток) до налогообложения отчетного периода (по 2016 год)|||t8042001|||Муниципальная собственность_Всего",
             "Прибыль (убыток) до налогообложения отчетного периода (по okved2)|||t8942001|||Муниципальная собственность_Всего по обследуемым видам экономической деятельности",
             "Задолженность поставщикам и подрядчикам за товары, работы и услуги из общей суммы кредиторской задолженности (по 2016 год)|||t8042008|||Муниципальная собственность_Всего",
             "Задолженность поставщикам и подрядчикам за товары, работы и услуги из общей суммы кредиторской задолженности (по okved2)|||t8942008|||Муниципальная собственность_Всего по обследуемым видам экономической деятельности",
             "Задолженность по платежам в государственные внебюджетные фонды из общей суммы кредиторской задолженности (по 2016 год)|||t8042007|||Муниципальная собственность_Всего",
             "Задолженность по платежам в государственные внебюджетные фонды из общей суммы кредиторской задолженности (по okved2)|||t8942007|||Муниципальная собственность_Всего по обследуемым видам экономической деятельности",
...
# create mun_debt
before_2016 <- BDMO_panel$t8042005_kfs14_okved0
after_2016 <- BDMO_panel$t8942005_kfs14_okved2101

mun_debt <- c()

for (i in 1:length(before_2016)) {
  if (is.na(before_2016[i])) {
    mun_debt <- c(mun_debt, after_2016[i])
  } else {
    mun_debt <- c(mun_debt, before_2016[i])
  }
}

BDMO_panel$mun_debt <- mun_debt


# create mun_debt_supl
before_2016 <- BDMO_panel$t8042008_kfs14_okved0
after_2016 <- BDMO_panel$t8942008_kfs14_okved2101

mun_debt_supl <- c()

for (i in 1:length(before_2016)) {
  if (is.na(before_2016[i])) {
    mun_debt_supl <- c(mun_debt_supl, after_2016[i])
  } else {
    mun_debt_supl <- c(mun_debt_supl, before_2016[i])
  }
}

BDMO_panel$mun_debt_supl <- mun_debt_supl


# create mun_debt_gov
before_2016 <- BDMO_panel$t8042007_kfs14_okved0
after_2016 <- BDMO_panel$t8942007_kfs14_okved2101

mun_debt_gov <- c()

for (i in 1:length(before_2016)) {
  if (is.na(before_2016[i])) {
    mun_debt_gov <- c(mun_debt_gov, after_2016[i])
  } else {
    mun_debt_gov <- c(mun_debt_gov, before_2016[i])
  }
}

BDMO_panel$mun_debt_gov <- mun_debt_gov


# create mun_debt_budg
before_2016 <- BDMO_panel$t8042006_kfs14_okved0
after_2016 <- BDMO_panel$t8942006_kfs14_okved2101

mun_debt_budg <- c()

for (i in 1:length(before_2016)) {
  if (is.na(before_2016[i])) {
    mun_debt_budg <- c(mun_debt_budg, after_2016[i])
  } else {
    mun_debt_budg <- c(mun_debt_budg, before_2016[i])
  }
}

BDMO_panel$mun_debt_budg <- mun_debt_budg


# create mun_debt_bank
before_2016 <- BDMO_panel$t8042009_kfs14_okved0
after_2016 <- BDMO_panel$t8942009_kfs14_okved2101

mun_debt_bank <- c()

for (i in 1:length(before_2016)) {
  if (is.na(before_2016[i])) {
    mun_debt_bank <- c(mun_debt_bank, after_2016[i])
  } else {
    mun_debt_bank <- c(mun_debt_bank, before_2016[i])
  }
}
